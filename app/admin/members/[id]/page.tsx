    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto pb-12 animate-fade-in">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <span className={`inline-block mt-1 text-xs font-semibold px-2.5 py-1 rounded-[10px] ${member.status === 'active' ? 'bg-green-50/50 text-ink' : member.status === 'suspended' ? 'bg-green-50/50 text-red' : 'bg-green-50/50 text-ink-2'}`}>
                {member.status}
              </span>
            </div>
          </div>
          <div className="flex gap-2">
            {member.status === 'active' && (
              <button onClick={() => setAction('suspend')} className="flex items-center gap-1.5 px-3 py-2 bg-green-50/50 text-red hover:bg-green-50/50 rounded-[10px] text-sm font-medium transition-colors">
                <UserX size={14} /> Suspend
              </button>
            )}
            {member.status === 'suspended' && (
              <button onClick={() => setAction('activate')} className="flex items-center gap-1.5 px-3 py-2 bg-green-50/50 text-ink hover:bg-green-50/50 rounded-[10px] text-sm font-medium transition-colors">
      ...
      {action && (
        <div className="border border-line rounded-[10px] p-5 mb-6 animate-slide-up">
          ...
          <textarea
            className="w-full px-3 py-2 bg-green-50/50 border border-line text-ink rounded-[10px] text-sm focus:outline-none focus:ring-0 focus:border-green mb-3"
          ...
          <div className="flex gap-2">
            <button onClick={() => handleStatusChange(action === 'suspend' ? 'suspended' : 'active')} disabled={processing}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 font-semibold rounded-[10px] text-sm transition-colors ${action === 'suspend' ? 'bg-red text-white' : 'bg-green text-white'}`}>
              {processing ? <Loader2 size={15} className="animate-spin" /> : 'Confirm'}
            </button>
            <button onClick={() => setAction(null)} className="px-4 py-2.5 bg-green-50/50 text-ink-2 hover:text-ink rounded-[10px] text-sm transition-colors">Cancel</button>
          </div>
        </div>
      )}
            ...
            <div className="border border-line rounded-[10px] p-5">
              ...
              <div className="space-y-2">
                {member.group_memberships.map((gm: any) => (
                  <div key={gm.id} className={`p-3 rounded-[10px] ${gm.status === 'defaulted' ? 'bg-green-50/50 border border-red/40' : 'bg-green-50/50'}`}>
        ...
        <div className="fixed inset-0 z-50 bg-ink/25 flex items-center justify-center p-4" onClick={() => setForfeitTarget(null)}>
          <div className="border border-line rounded-[10px] w-full max-w-md p-6 space-y-4 animate-slide-up" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-[10px] bg-green-50/50 flex items-center justify-center">
            ...
            <div className="p-3 bg-green-50/50 border border-red/40 rounded-[10px]">
            ...
            <div>
              ...
              <textarea rows={2} value={forfeitReason} onChange={e => setForfeitReason(e.target.value)}
                className="w-full px-3 py-2 bg-green-50/50 border border-line text-ink rounded-[10px] text-sm focus:outline-none focus:ring-0 focus:ring-red-500 resize-none"
