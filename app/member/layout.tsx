    ...
    <div className="min-h-screen">
      ...
      <nav className="fixed bottom-0 inset-x-0 z-40 pb-[env(safe-area-inset-bottom)]">
        <div className="max-w-[420px] mx-auto px-5 pb-4">
          <div className="flex items-center gap-1 p-1.5 bg-surface/95 backdrop-blur-lg rounded-[16px] border border-line"
              ...
              return (
                <Link key={href} href={href}
                  className={`flex-1 flex items-center justify-center gap-2 h-11 rounded-[11px] text-[13px] font-semibold transition-all ${
                    on ? 'bg-green text-white' : 'text-ink-2 hover:bg-green-50'
                  }`}>
                  <Icon size={16} />{label}
                </Link>
              )
