-- ============================================================================
-- V32 — DIRECTED SETTLEMENT: one engine, two queue sources  (Phase 04)
-- ============================================================================
-- `payments-manual` records money an admin collected by hand — cash, MoMo sent
-- direct, bank transfer. It is by far the busiest path in this platform:
-- 3,978 contributions and GHS 403,940 of collections.
--
-- It is also the last duplicate settlement implementation. It does a blanket
-- UPDATE, writes no allocation rows, never sets amount_paid, and never touches
-- credit.
--
-- ── WHY IT CANNOT SIMPLY CALL settle_payment() ───────────────────────────
-- The two answer different questions.
--
--   settle_payment()  "GHS 450 arrived — apply it by the allocation policy."
--                     The engine chooses which days, oldest first.
--
--   manual payment    "The admin asserts THESE SPECIFIC DAYS were paid."
--                     The days are the input, not the output.
--
-- Routing manual collection through the policy queue would silently change
-- admin behaviour: an admin ticking days 5, 7 and 9 would find days 1, 2 and 3
-- settled instead. On a path carrying GHS 403,940 that is not an acceptable
-- side effect of a refactor.
--
-- ── THE FIX ──────────────────────────────────────────────────────────────
-- Not a second engine — a second QUEUE SOURCE for the same one. When
-- p_target_contributions is supplied, the queue is exactly those obligations
-- in due-date order; otherwise it is the policy queue as before. Everything
-- after that point is identical: the same locking, the same allocation
-- records, the same credit ledger, the same atomicity, the same audit.
--
-- One implementation of "money meets obligation". Two ways of deciding which
...
DROP FUNCTION IF EXISTS settle_payment(TEXT, DECIMAL, TEXT, DATE, UUID[]);
...
CREATE FUNCTION settle_payment(
  ...
  -- When given, settle EXACTLY these obligations in due-date order and ignore
  -- the allocation policy. Used only by directed (manual) collection.
...
BEGIN
  ...
  -- 2. Idempotency: a replay returns what was recorded and writes nothing.
  ...
  WHERE c.status <> 'paid'
    ...
    AND (
      -- Directed: exactly the obligations the caller named.
      (v_directed AND c.id = ANY(p_target_contributions))
      -- Policy: the member's obligations, filtered by scope.
      OR (NOT v_directed AND (
            p_scope = 'member'
  ...
  -- 4. Allocate. Directed settlement ignores origin ranking — the caller
  --    already decided the set, so due-date order is the only ordering left.
  FOR v_ob IN
    SELECT * FROM _queue
...
COMMENT ON FUNCTION settle_payment(TEXT, DECIMAL, TEXT, DATE, UUID[]) IS
  'The one place money is applied to obligations. Atomic, row-locked, '
  'idempotent. Two queue sources: the allocation policy, or a directed list of '
  'obligations for admin-recorded manual collection.';

REVOKE ALL ON FUNCTION settle_payment(TEXT, DECIMAL, TEXT, DATE, UUID[]) FROM PUBLIC;
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='anon') THEN
    EXECUTE 'REVOKE ALL ON FUNCTION settle_payment(TEXT,DECIMAL,TEXT,DATE,UUID[]) FROM anon';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='authenticated') THEN
    EXECUTE 'REVOKE ALL ON FUNCTION settle_payment(TEXT,DECIMAL,TEXT,DATE,UUID[]) FROM authenticated';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='service_role') THEN
    EXECUTE 'GRANT EXECUTE ON FUNCTION settle_payment(TEXT,DECIMAL,TEXT,DATE,UUID[]) TO service_role';
  END IF;
END $$;
